<div class="kp-upi-template-3-qr-card">
	<div class="header">
		<h2><?php echo esc_html( $payee_name ); ?></h2>
		<p>Scan to Pay ₹<?php echo esc_html( $intent_url_parameters['am'] ); ?></p>
	</div>
	<div class='qrCodeWrapper' id='qrCodeWrapper' style='display: none;'>
			<?php if ( $hide_pay_button ) { ?>
			<div class='qr-code qrCodeBody'></div>
			<?php } else { ?>
			<a href='<?php echo esc_url( add_query_arg( $intent_url_parameters, 'upi://pay' ) ); ?>'>
				<div class='qr-code qrCodeBody'></div>
			</a>
			<?php } ?>
	</div>
